...
func TestMainNoArgsPrintsUsageAndExitsOne(t *testing.T) {
	...
	if !strings.Contains(stdout, "haisdk - HAI SDK CLI for agent management") {
...
func TestMainHelpPrintsUsageAndExitsZero(t *testing.T) {
	...
	if !strings.Contains(stdout, `Use "haisdk <command> --help" for more information.`) {
...
func TestCLIHelperProcess(t *testing.T) {
	...
	os.Args = append([]string{"haisdk"}, os.Args[dashIdx+1:]...)
	main()
	os.Exit(0)
}
