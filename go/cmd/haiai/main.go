// Command haiai provides a CLI for HAI agent management.
//
// Usage:
//
//	haiai register --name "My Agent" --description "..." --dns "agent.example" --owner-email "user@example.com"
//	haiai hello
//	haiai benchmark --tier free
//	haiai verify --jacs-id <id>
//	haiai check-username --username <name>
//	haiai claim-username --username <name> --agent-id <id>
//	haiai send-email --to <addr> --subject <subj> --body <body>
//	haiai list-messages
package main
...
func printUsage() {
	fmt.Println(`haiai - HAI SDK CLI for agent management
...
Use "haiai <command> --help" for more information.`)
}

func newClient() *haiai.Client {
	cl, err := haiai.NewClient()
	if err != nil {
...
func cmdRegister(args []string) {
	...
	opts := &haiai.RegisterNewAgentOptions{
	...
	if endpoint == "" {
		endpoint = haiai.DefaultEndpoint
	}
	result, err := haiai.RegisterNewAgentWithEndpoint(context.Background(), endpoint, *name, opts)
	if err != nil {
...
func cmdSendEmail(args []string) {
	...
	cl := newClient()
	result, err := cl.SendEmailWithOptions(context.Background(), haiai.SendEmailOptions{
		To:        *to,
		Subject:   *subject,
		Body:      *body,
		InReplyTo: *inReplyTo,
...
func cmdListMessages(args []string) {
	...
	cl := newClient()
	messages, err := cl.ListMessages(context.Background(), haiai.ListMessagesOptions{
		Limit:     *limit,
		Offset:    *offset,
		Direction: *direction,
	})
