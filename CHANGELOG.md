...
- **2026-09-04: Explicit live response context.** HAI streams and job responses use JACS's closed response-v2 context union with configured tenant/audience, authenticated connection binding, exact job causation, and replay checks. Rust, Python, Node, and Go expose matching expected-context configuration through shared FFI initialization. Legacy signature inspection is unchanged; action-accepting consumers require the new context.
