...
### Self-knowledge embedding

The MCP tool `hai_self_knowledge` and CLI command `haiai self-knowledge` search embedded documentation using BM25. Documents are compiled into the binary via `include_str!()` -- no runtime file I/O.

**What's embedded** (104 documents):
- JACS jacsbook chapters (parsed from `../JACS/jacs/docs/jacsbook/src/SUMMARY.md`)
- SDK READMEs (root, Python, Node, Go, Rust library, CLI, MCP, fixtures)
- Project guides (`DEVELOPMENT.md`, `AGENTS.md`, `CLAUDE.md`, language sync guide, parity map, ADRs, SKILL.md)
- JSON schemas (`schemas/*.json`)
- JACS root README

**Regenerating** (requires sibling `../JACS` repo):
```bash
./scripts/generate_knowledge.sh    # regenerate docs + self_knowledge_data.rs
make check-knowledge               # fail if self_knowledge_data.rs is stale
```

**When to regenerate:**
- After editing `DEVELOPMENT.md`, `AGENTS.md`, `CLAUDE.md`, or any doc in `docs/`
- After updating JACS jacsbook content
- After adding/changing SDK READMEs or JSON schemas
- Before release (`make check-knowledge` catches staleness)

The freshness check runs `generate_knowledge.sh`, diffs the output against the committed `self_knowledge_data.rs`, and fails if they differ. It restores the original file on failure so your working tree stays clean.
