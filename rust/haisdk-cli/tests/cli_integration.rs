...
fn help_flag_exits_zero() {
    let output = Command::new(haisdk_bin())
        .arg("--help")
        .output()
        .expect("run --help");
    assert!(output.status.success(), "exit code: {}", output.status);
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("HAISDK CLI"), "stdout: {stdout}");
...
fn init_creates_config_keys_and_prints_agent_id() {
    ...
    assert!(stdout.contains("haisdk mcp"), "should hint about mcp: {stdout}");
...
impl McpSession {
    ...
    fn read_message(&mut self) -> Value {
        loop {
            ...
            assert!(read > 0, "haisdk mcp closed stdout unexpectedly");
